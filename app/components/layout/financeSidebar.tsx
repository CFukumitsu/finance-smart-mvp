...
export default function FinanceSidebar({
  ...
  return (
    <aside
      ...
      className={`h-screen shrink-0 border-r border-white/10 bg-slate-950/95 px-3 py-5 shadow-2xl shadow-black/30 transition ${isExpanded ? "w-72" : "w-20"
        }`}
    >
        ...
        <div className="mb-8 flex items-center justify-between gap-3">
          ...
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-red-500/20 bg-red-500/10">
            ...
            {isExpanded && (
              <div>
                ...
                <p className="text-xs text-slate-500">Powered by FKT Systems</p>
              </div>
            )}
          </div>
          ...
          {isExpanded && !forceExpanded && (
            <button
              ...
              className={`flex h-10 w-10 items-center justify-center rounded-xl border text-sm font-bold transition ${isPinned
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={onNavigate}
                className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-semibold transition ${isActive
