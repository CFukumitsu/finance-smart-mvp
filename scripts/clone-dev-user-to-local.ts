...
async function fetchPaged(
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    const { data, error } = await filter(base).range(
      from,
      from + PAGE_SIZE - 1,
    );
...
async function fetchForOwner(
  ...
  if (spec.ownership === "direct") {
    return fetchPaged(database, spec.table, (query) =>
  ...
  for (let offset = 0; offset < accountIds.length; offset += 100) {
    ...
    rows.push(
      ...(await fetchPaged(database, spec.table, (query) =>
        query.in("account_id", ids),
      )),
    );
  }
  return rows;
...
async function loadOwnedData(
  ...
  const rows = Object.fromEntries(
    TABLE_SPECS.map((spec: (typeof TABLE_SPECS)[number]) => [spec.table, []]),
  ) as unknown as RowsByTable;
...
function rowsToInsert(
  ...
  return source[spec.table]
    ...
    .filter(
      (row) =>
        spec.table !== "competences" || !reusedCompetences.has(String(row.id)),
    );
}
...
async function removeInserted(
  ...
  if (failures.length)
    throw new Error(`Limpeza local incompleta: ${failures.join(" | ")}`);
}

async function main(): Promise<void> {
  const execute = process.argv.includes("--execute");
  const ownerIndex = process.argv.indexOf("--local-owner-id");
  const localOwnerId = required(
    "--local-owner-id",
    process.argv[ownerIndex + 1],
  );
  if (
  ...
  const fileVariables = parseCloneEnvFile(
    await readFile(resolve(".env.clone.local"), "utf8"),
  );
  const devUrl = required(
    "DEV_SUPABASE_URL",
    process.env.DEV_SUPABASE_URL ?? fileVariables.DEV_SUPABASE_URL,
  );
  const devKey = required(
    "DEV_SUPABASE_SERVICE_ROLE_KEY",
    process.env.DEV_SUPABASE_SERVICE_ROLE_KEY ??
      fileVariables.DEV_SUPABASE_SERVICE_ROLE_KEY,
  );
  const devOwnerId = required(
    "DEV_OWNER_ID",
    process.env.DEV_OWNER_ID ?? fileVariables.DEV_OWNER_ID,
  );
  const localUrl = required(
    "LOCAL_SUPABASE_URL",
    process.env.LOCAL_SUPABASE_URL,
  );
  const localKey = required(
  ...
  if (new URL(devUrl).hostname !== `${DEV_PROJECT_REF}.supabase.co`) {
    throw new Error(
      `Origem recusada: deve ser exclusivamente o projeto DEV ${DEV_PROJECT_REF}.`,
    );
  }
  ...
  const [
    { data: sourceUser, error: sourceUserError },
    { data: localUser, error: localUserError },
  ] = await Promise.all([
    sourceClient.auth.admin.getUserById(devOwnerId),
    localClient.auth.admin.getUserById(localOwnerId),
  ]);
  if (sourceUserError || !sourceUser.user)
    throw new Error("Proprietário configurado não existe no DEV.");
  if (localUserError || !localUser.user)
    throw new Error("Proprietário informado não existe no Supabase LOCAL.");
  ...
  if (destinationState.functionalCount !== 0) {
    throw new Error(
      `Destino local possui ${destinationState.functionalCount} registro(s) funcional(is); restauração recusada para evitar duplicação.`,
    );
  }
  if (!source.accounts.length || !source.transactions.length) {
    throw new Error(
      "A cópia no DEV não possui contas ou lançamentos para o proprietário configurado.",
    );
  }

  const { maps, reusedCompetenceIds } = buildUuidMaps(
    source,
  ...
  const prepared = Object.fromEntries(
    TABLE_SPECS.map((spec) => [
      spec.table,
      rowsToInsert(spec, source, maps, localOwnerId, reused),
    ]),
  ) as RowsByTable;
  console.table(
    TABLE_SPECS.map((spec) => ({
  ...
  console.log(
    `Competências locais reutilizadas: ${reusedCompetenceIds.length}.`,
  );
  ...
  try {
    for (const spec of TABLE_SPECS) {
      ...
      for (let offset = 0; offset < rows.length; offset += WRITE_BATCH_SIZE) {
        ...
        if (error)
          throw new Error(`Falha ao inserir ${spec.table}: ${error.message}`);
    ...
    for (const spec of TABLE_SPECS) {
      const expected =
        destination[spec.table].length + prepared[spec.table].length;
      if (finalRows[spec.table].length !== expected) {
        throw new Error(
          `${spec.table}: esperado ${expected}, encontrado ${finalRows[spec.table].length}.`,
        );
      }
    }
    ...
    console.error(
      "Falha durante a restauração; removendo somente os UUIDs inseridos nesta execução.",
    );
