...
export default function UsersPageContent() {
  ...
  async function changeRole(userId: string, role: UserRole) {
    return mutation.run("changeRole" + ":" + (userId), async () => {
...
function RoleSelect({ user, changeRole, busy, disabled }: { busy: boolean; disabled: boolean; user: AdminUserListItem; changeRole: (id: string, role: UserRole) => void }) {
  const mutation = useMutation();
  const [resending, setResending] = useState(false);
  async function resend() {
    return mutation.run("resend", async () => {
      try {
  ...
  return <div className="space-y-2">{busy && <span role="status" className="text-xs text-slate-400">Processando...</span>}<select aria-label={`Perfil de ${user.email}`} value={user.role} disabled={disabled || mutation.isPending || user.role === "manager"} onChange={(event) => changeRole(user.id, event.target.value as UserRole)} className="w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm disabled:text-slate-400"><option value="user">Usuário</option><option value="admin">Administrador</option>{user.role === "manager" && <option value="manager">Gerente</option>}</select>{user.status === "invited" && <ProcessingButton busy={mutation.pending === ("resend")} type="button" disabled={disabled || mutation.isPending || (resending)} onClick={resend} className="w-full rounded-lg border border-blue-400/20 px-3 py-2 text-xs font-semibold text-blue-300 hover:bg-blue-500/10 disabled:opacity-50">{resending ? "Reenviando..." : "Reenviar convite"}</ProcessingButton>}</div>;
}
...
function InviteModal({ close, onSuccess }: { close: () => void; onSuccess: (message: string) => Promise<void> }) {
  const mutation = useMutation();
  useModalShortcuts({ enabled: true, isBlocked: mutation.isLocked, onEscape: close });
