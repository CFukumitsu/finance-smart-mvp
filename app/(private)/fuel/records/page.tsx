...
export default function FuelRecordsPage() {
  ...
  const filtered = useMemo(() => records.filter((r) => {
    const text = `${r.vehicle?.name} ${r.station?.name} ${r.fuel_type} ${r.transaction?.description}`.toLowerCase();
    return (!search || text.includes(search.toLowerCase())) && (!vehicle || r.vehicle_id === vehicle) && (!station || r.fuel_station_id === station) && (!fuel || r.fuel_type === fuel) && (!fullTank || String(r.full_tank) === fullTank) && (!month || r.recorded_at.startsWith(month));
  }).sort((a,b) => b.recorded_at.localeCompare(a.recorded_at)), [records, search, vehicle, station, fuel, fullTank, month]);
  ...
  return <AppShell><div className="space-y-6"><div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold text-white">Abastecimentos</h1><p className="text-sm text-slate-400">Histórico integrado aos lançamentos financeiros.</p></div><Link href="/transactions?new=fuel" className="rounded-xl bg-amber-500 px-4 py-3 font-semibold text-slate-950">Novo abastecimento</Link></div>
    ...
    <select value={station} onChange={e=>setStation(e.target.value)} className="rounded-lg bg-slate-950 p-3 text-white"><option value="">Todos os postos</option>{unique(records.map(r=>r.fuel_station_id).filter((id): id is string=>id!==null)).map(id=><option key={id} value={id}>{records.find(r=>r.fuel_station_id===id)?.station?.name}</option>)}</select>
