...
import { MutationScope, ProcessingButton } from "@/src/components/ui/Mutation";
...
export default function AccountPageContent() {
  ...
  async function save(event: React.FormEvent) {
    ...
    return mutation.run("save", async () => {
      try {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <MutationScope busy={mutation.isPending} isLocked={mutation.isLocked}><form onSubmit={save} className="space-y-6 rounded-3xl border border-white/10 bg-slate-900/50 p-5 sm:p-7">
          ...
          {loading ? <p className="text-slate-400">Carregando perfil...</p> : <>
            ...
            <ProcessingButton busy={mutation.pending === ("save")} type="submit" disabled={mutation.isPending || (saving || !profile)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-bold hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"><Save size={18} />{saving ? "Salvando..." : "Salvar alterações"}</ProcessingButton>
          </>}
        </form></MutationScope>
      </div>
    </AppShell>
