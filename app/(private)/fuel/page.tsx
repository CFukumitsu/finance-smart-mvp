...
export default function FuelPage() {
  ...
  const month=new Date().toISOString().slice(0,7); const current=useMemo(()=>records.filter(r=>r.recorded_at.startsWith(month)),[records,month]); const cycles=useMemo(()=>calculateFuelCycles(records),[records]); const latest=[...records].sort((a,b)=>b.recorded_at.localeCompare(a.recorded_at))[0];
  const summary=[['Veículo padrão',defaultVehicle],['Último abastecimento',latest?`${latest.vehicle?.name} · ${new Date(`${latest.recorded_at}T12:00:00`).toLocaleDateString('pt-BR')}`:'Nenhum'],['Gasto no mês',`R$ ${formatFuelNumber(current.reduce((s,r)=>s+r.total_value,0))}`],['Litros no mês',formatFuelNumber(current.reduce((s,r)=>s+r.liters,0),3)],['Consumo médio',cycles.length?`${formatFuelNumber(cycles.reduce((s,c)=>s+c.consumptionKmPerLiter,0)/cycles.length)} km/l`:'—'],['Custo por km',cycles.length?`R$ ${formatFuelNumber(cycles.reduce((s,c)=>s+c.costPerKm,0)/cycles.length,3)}`:'—']];
  return (
    <AppShell>
